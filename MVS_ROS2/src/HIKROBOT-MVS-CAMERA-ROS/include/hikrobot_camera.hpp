...
#define MAX_IMAGE_DATA_SIZE (4 * 2048 * 3072)
    ...
    public:
        //********** 构造函数  ****************************/
        Camera(const rclcpp::Node::SharedPtr &node);
        //********** 析构函数  ****************************/
        ~Camera();
        //********** 原始信息转换线程 **********************/
    ...
    //^ ********************************** Camera constructor************************************ //
    Camera::Camera(const rclcpp::Node::SharedPtr &node)
    {
        handle = NULL;
        node_ptr_ = node;
        ...
        // try {
        //     node_ptr_->get_parameter("width", width);
        // } catch (...) { width = 1280; }
        // RCLCPP_WARN(node->get_logger(), "width:%d", width);
        // try {
        //     node_ptr_->get_parameter("height", height);
        // } catch (...) { height = 1024; }
        // RCLCPP_WARN(node->get_logger(), "height:%d", height);
        // try {
        //     node_ptr_->get_parameter("FrameRateEnable", FrameRateEnable);
        // } catch (...) { FrameRateEnable = false; }
        // try {
        //     node_ptr_->get_parameter("FrameRate", FrameRate);
        // } catch (...) { FrameRate = 10; }
        // RCLCPP_WARN(node->get_logger(), "FrameRate:%d", FrameRate);
        // try {
        //     node_ptr_->get_parameter("BurstFrameCount", BurstFrameCount);
        // } catch (...) { BurstFrameCount = 10; } // 一次触发采集的次数
        // try {
        //     node_ptr_->get_parameter("ExposureTime", ExposureTime);
        // } catch (...) { ExposureTime = 12000; }
        // ExposureTime = 12000;
        // RCLCPP_WARN(node->get_logger(), "ExposureTime:%d", ExposureTime);
        // try {
        //     node_ptr_->get_parameter("GammaEnable", GammaEnable);
        // } catch (...) { GammaEnable = false; }
        // try {
        //     node_ptr_->get_parameter("Gamma", Gamma);
        // } catch (...) { Gamma = (float)0.7; }
        // try {
        //     node_ptr_->get_parameter("GainAuto", GainAuto);
        // } catch (...) { GainAuto = 2; }
        // try {
        //     node_ptr_->get_parameter("SaturationEnable", SaturationEnable);
        // } catch (...) { SaturationEnable = true; }
        // try {
        //     node_ptr_->get_parameter("Saturation", Saturation);
        // } catch (...) { Saturation = 128; }
        // try {
        //     node_ptr_->get_parameter("Offset_x", Offset_x);
        // } catch (...) { Offset_x = 0; }
        // try {
        //     node_ptr_->get_parameter("Offset_y", Offset_y);
        // } catch (...) { Offset_y = 0; }
        // try {
        //     node_ptr_->get_parameter("TriggerMode", TriggerMode);
        // } catch (...) { TriggerMode = 1; }
        ...
        FrameRate = 5;           // 帧率
        ExposureTime = 1000;     // 曝光时间(微秒)
        GammaEnable = true;       // 使能伽马校正
        Gamma = 0.7f;             // 伽马因子
        GainAuto = 2;             // 亮度自动增益模式
        SaturationEnable = false; // 禁用饱和度调整
        Saturation = 128;         // 饱和度值
        TriggerMode = 1;          // 触发模式
        TriggerSource = 2;        // 触发源

        // 打印固定参数配置
        RCLCPP_INFO(node_ptr_->get_logger(), "相机参数配置(硬编码):");
        RCLCPP_INFO(node_ptr_->get_logger(), "  width: %d", width);
        RCLCPP_INFO(node_ptr_->get_logger(), "  height: %d", height);
        RCLCPP_INFO(node_ptr_->get_logger(), "  Offset_x: %d", Offset_x);
        RCLCPP_INFO(node_ptr_->get_logger(), "  Offset_y: %d", Offset_y);
        RCLCPP_INFO(node_ptr_->get_logger(), "  FrameRate: %d FPS", FrameRate);
        RCLCPP_INFO(node_ptr_->get_logger(), "  ExposureTime: %d μs", ExposureTime);
        RCLCPP_INFO(node_ptr_->get_logger(), "  Gamma: %.1f", Gamma);
        RCLCPP_INFO(node_ptr_->get_logger(), "  GainAuto: %d", GainAuto);
        RCLCPP_INFO(node_ptr_->get_logger(), "  Saturation: %d", Saturation);
        RCLCPP_INFO(node_ptr_->get_logger(), "  TriggerMode: %d", TriggerMode);
        RCLCPP_INFO(node_ptr_->get_logger(), "  TriggerSource: %d", TriggerSource);


        try {
            node_ptr_->get_parameter("TriggerSource", TriggerSource);
        } catch (...) { TriggerSource = 2; }
        try {
            node_ptr_->get_parameter("LineSelector", LineSelector);
        } catch (...) { LineSelector = 2; }
