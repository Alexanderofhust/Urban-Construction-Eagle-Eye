...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstPtr &msg)
{
  ...
  while(lid_raw_data_buffer.size() > 100)
  {
    lid_raw_data_buffer.pop_front();
    lid_header_time_buffer.pop_front();
    RCLCPP_ERROR(nh_->get_logger(), "Lidar buffer size exceeds 100, removing oldest data.");
  }
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstPtr &msg_in)
{
  ...
  if (last_timestamp_lidar < 0.0)
    return;
  // rclcpp::Time ros2_time(msg_in->header.stamp.sec, msg_in->header.stamp.nanosec);
  // RCLCPP_INFO(nh_->get_logger(), "get imu at time: %.6f", ros2_time.seconds());
  ...
  //   RCLCPP_ERROR(nh_->get_logger(), "IMU buffer size exceeds 100, removing oldest data.");
  // }
  sig_buffer.notify_all();
}
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstPtr &msg_in)
{
  ...
  while(img_buffer.size() > 100)
  {
    img_buffer.pop_front();
    img_time_buffer.pop_front();
    RCLCPP_WARN(nh_->get_logger(), "Image buffer size exceeds 100, pop the oldest image.");
  }
